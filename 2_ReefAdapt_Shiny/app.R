...
insideRangepolyCheck <- function(click, rangepoly) {
  # Assuming rangepoly is a polygon and click is a point
  # You can use spatial functions to check if the point is inside the polygon
  # For example, using sf package:
  point <- sf::st_point(c(click$lng, click$lat))
  polygon <- sf::st_as_sf(rangepoly, coords = c("lng", "lat"), crs = 4326)
  return(sf::st_contains(polygon, point, sparse = FALSE)[1,1])
}
...
server <- function(input, output, session) {
  ...
  # Create needed reactiveValues to keep track of selected and created values
  created_values <- reactiveValues(rangepoly = NULL, rest.site = NULL, range=NULL,refid=NULL)
  ...
  observeEvent(input$calc, {
    ...
    if (!is.null(created_values$rest.site)){
      ...
      refid <- created_values$refid
      #print(year)
      
      
      if (!is.null(created_values$rangepoly)) {
        # Check if click is inside rangepoly
        print('inside 1')
        if (!insideRangepolyCheck(input$ReefAdapt_click, created_values$rangepoly)) {
          # Raise an error message if click is outside the rangepoly
          showModal(modalDialog(
            title = "Error",
            "The click must be inside the range polygon.",
            easyClose = TRUE,
            footer = NULL
          ))
      ...
      if (year == "2020") {
        ...
        leafletProxy('ReefAdapt') %>%
          clearGroup('green_circles') %>%
          clearImages() %>%
          addPolygons(data =  cropped_poly, fillColor = "limegreen", fillOpacity = 0.3,group='green_circles',
      ...
      if (year == "2050") {
        ...
        if (all(is.na(values_string))) {
          ...
          leafletProxy('ReefAdapt') %>%
            clearGroup('red_circles') %>%
            clearImages() %>%
            addPolygons(data =  cropped_poly, fillColor = "red", fillOpacity = 0.3,group = 'red_circles',
  ...
  observeEvent(input$show_survey_gaps, {
    ...
    if (input$show_survey_gaps) {
      ...
      gaps <- readRDS(gap2)
      extentR <- extent(created_values$Range)
      ...
      leafletProxy('ReefAdapt') %>%
        ...
        addCircles(data = ref_sites, group='survey_circles',
  ...
  observeEvent(input$ReefAdapt_click, {
    ...
    # Check if rangepoly is initialized
    if (!is.null(created_values$rangepoly)) {
      # Check if click is inside rangepoly
      print('inside 1')
      if (!insideRangepolyCheck(input$ReefAdapt_click, created_values$rangepoly)) {
        ...
        print('inside where we should be')
        leafletProxy('ReefAdapt') %>%
          clearGroup('circles') %>%
          clearImages() %>%
        ...
        updateSelectInput(session, "year_selection", label = "Select a year:",
                          choices = c("", "2020", "2050"))
        updateTextInput(session, "lat_selection", label = "Latitude",
                        value = clat)
        updateTextInput(session, "lon_selection", label = "Longitude",
                        value = clng)
        ...
        created_values$rest.site <- rest.site
        rest.site <- SpatialPointsDataFrame(coords = rest.site, proj4string = crs(created_values$Range), data = rest.site)
